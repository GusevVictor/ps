#### Create Disk ####
resource "openstack_blockstorage_volume_v3" "vm-01" {
  name                 = "vm-01"
  size                 = "6"
...
#### Create Instanse ####
resource "openstack_compute_instance_v2" "vm-01" {
  name             = "vm-01"
  ...
  depends_on = [
                ...
                openstack_blockstorage_volume_v3.vm-01
]  
  network {
    uuid           = openstack_networking_network_v2.private_network.id
          }
  block_device {
    uuid           = openstack_blockstorage_volume_v3.vm-01.id
    boot_index     = 0
    source_type    = "volume"
